...
function findCardById(columns, cardId, columnId) {
    const column = columns.find(x => x.id == columnId);
    const card = column.cards.find(x => x.id == cardId);
    return card;
}

function findCommentById(columns, commentId) {

}

export const columnActions = boardSlice.actions;
